...
class hefengWeather(WeatherEntity):
    def __init__(self, api_key: str, location: str, name: str, default) -> None:
        ...
        self._air_data = None
        self.now_url = f"https://devapi.hefengWeather.com/v7/weather/now?location={self.location}&key={self.api_key}"
        self.daily_url = f"https://devapi.hefengWeather.com/v7/weather/{self.default}d?location={self.location}&key={self.api_key}"
        self.indices_url = f"https://devapi.hefengWeather.com/v7/indices/1d?type=0&location={self.location}&key={self.api_key}"
        self.air_url = f"https://devapi.hefengWeather.com/v7/air/now?location={self.location}&key={self.api_key}"
        self.hourly_url = f"https://devapi.hefengWeather.com/v7/weather/24h?location={self.location}&key={self.api_key}"
        self.minutely_url = f"https://devapi.hefengWeather.com/v7/minutely/5m?location={self.location}&key={self.api_key}"
        self.warning_url = f"https://devapi.hefengWeather.com/v7/warning/now?location={self.location}&key={self.api_key}"
...
#         return self._current["text"]
    ...
    def state_attributes(self):
        """注册自定义的属性"""
        data = super(hefengWeather, self).state_attributes
